...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        Node<K, V> *z;
        z = new Node <K, V>(key, key_value.second);
        while(x!= nullptr){
        	if(key== x-> key() ){
        		std::stringstream err;
        		err << key;
        		throw tree_exception("Attempt to insert duplicate key '"+err.str()+ "'.");
        		err.clear();
        	}
...
private:
    ...
    size_t leaf_count(Node<K, V> *node) const {
		...
		if (node-> left!= nullptr){
			numLeaves+= leaf_count(node->left);
		}
		if (node-> right!= nullptr){
			numLeaves+= leaf_count(node->right);
		}
		return numLeaves;
    }
    ...
    size_t internal_node_count(Node<K, V> *node) const {
    	...
    	if(node-> left!=nullptr||node-> right!=nullptr){
    		notNull++;
    		internalNodes+= internal_node_count(node-> left)+internal_node_count(node-> right);
    	}
    	else{
    		internalNodes+= internal_node_count(node-> left)+internal_node_count(node-> right);
    	}
    	return notNull + internalNodes;
    ...
    int diameter(Node<K, V> *node) const {
		...
		int diameter;
		int heightTotal = height(node-> left)+ height(node-> right) +3;
